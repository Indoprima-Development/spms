...
<body>
            ...
            <div class="container-fluid">
                ...
                <div class="card bg-light-info shadow-none position-relative overflow-hidden">
                    <div class="card-body px-4 py-3">
                        <div class="row align-items-center">
                            <div class="col-9">
                                <h4 class="fw-semibold mb-8">Sparepart Management System</h4>
                ...
                <section class="datatables">
                    ...
                    <div class="card">
                        <div class="card-body">
                            <div class="mb-2">
                                <h5 class="mb-0">List Sparepart Mesin</h5>
                            </div>
                            <p class="card-subtitle mb-3"></p>
                            <div class="table-responsive m-t-40">
                                <table id="checksheets-table"
                                    class="table border display table-bordered table-striped no-wrap nowrap"
                                    style="width: 100%">
                                    <thead>
                                        <!-- start row -->
                                        <tr>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            axios.get('http://35.224.199.133:5000/api/spms')
                .then(function(response) {
                    const checksheets = response.data.data; // Akses data dari response
                    const dataTable = new DataTable('#checksheets-table', {
                        scrollX: true
                    });
                    dataTable.clear(); // Clear existing data
                    checksheets.forEach(function(checksheet) {
                        dataTable.row.add([
                            checksheet.ItemNumber || 'N/A',
                            checksheet.ProductName || 'N/A',
                            checksheet.Unit || 'N/A',
                            checksheet.KategoriGolongan || 'N/A',
                            checksheet.UnitMesin || 'N/A',
                            checksheet.Kategori || 'N/A',
                            checksheet.SafetyStock || 'N/A',
                            checksheet.Qty || 'N/A',
                            checksheet.Location.replace('\\N\r', 'N/A'),
                            `<button class="btn btn-warning btn-sm btn-edit" data-id="${checksheet.id}">Edit</button>
                    <button class="btn btn-danger btn-sm" data-id="${checksheet.id}">Delete</button>`
                        ]).draw(); // Add row and draw table
                    });

                    // Event delegation for Edit and Delete buttons
                    $('#checksheets-table').on('click', '.btn-edit', function() {
                        const id = $(this).data('id');
                        // Handle edit logic here
                        console.log('Edit item with ID:', id);
                    });

                    $('#checksheets-table').on('click', '.btn-danger', function() {
                        const id = $(this).data('id');
                ...
                .catch(function(error) {
                    ...
                    const dataTable = $('#checksheets-table').DataTable();
                    dataTable.clear();
                    dataTable.row.add([
                        'Error fetching data', '', '', '', '', '', '', '', '', ''
                    ]).draw();
